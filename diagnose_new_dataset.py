...
DATASET = r"c:\Users\hp\Desktop\orchestration_agent\dataset\new_dataset.csv"
...
def norm(lbl):
    n=(lbl or "").strip().lower()
    return ALIAS_MAP.get(n, next((v for k,v in ALIAS_MAP.items() if n.startswith(k)), "out_of_scope"))
...
preds   = [_normalize_intent(p) for p in preds]
